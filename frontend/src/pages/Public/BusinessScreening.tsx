...
const BusinessScreeningPage: React.FC = () => {
  ...
  return (
    <div className="business-screening-page" style={shellStyle}>
      <div className="business-screening-container" style={{ maxWidth: 880, margin: '0 auto', height: '100%', display: 'flex', flexDirection: 'column' }}>
        <div style={{ ...cardStyle, padding: 14, display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, overflow: 'hidden' }}>
          <div style={{ marginBottom: 12, flexShrink: 0 }}>
            <h1 style={{ margin: 0, fontSize: 22 }}>{data?.batch.title || '业务筛选'}</h1>
            <p style={{ margin: '4px 0 8px', color: '#64748b', fontSize: 13 }}>{data?.batch.subtitle || '请查看候选人信息并完成入库 / 不入库决策。'}</p>
            <div className="business-screening-meta" style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
              <span className="business-screening-meta-pill" style={{ borderRadius: 999, background: '#eff6ff', color: '#1d4ed8', padding: '4px 10px', fontSize: 13 }}>
                {data?.batch.interviewer || '待分配'}
              </span>
              <span className="business-screening-meta-pill" style={{ borderRadius: 999, background: '#f1f5f9', color: '#334155', padding: '4px 10px', fontSize: 13 }}>
          ...
          {data?.resumes.length ? (
            ...
            >
              <section className="business-screening-candidates" style={{ ...cardStyle, padding: 8, minWidth: 0, overflowY: 'auto' }}>
                <h2 style={{ margin: '2px 4px 8px', fontSize: 15 }}>候选人列表</h2>
