...
import { Spin, Tag, Progress, Descriptions, Button, message, Form, Input, Radio } from 'antd';
import {
...
/** 面试评价（面试官在卡片链接内填写一面/二面评价与结果；无面试记录也可提交一面，自动创建面试记录） */
function EvaluationSection({
  ...
  // 无面试记录 → 提供一面评价（提交时自动创建面试记录）；有记录按当前轮次
  const targetRound = primary?.id ? resolveTargetRound(primary) : 1;
  if (primary?.id && targetRound === null) {
    return (
      <div style={sectionCardStyle}>
