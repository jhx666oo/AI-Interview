...
当查询结果需要生成业务筛选链接时，先根据用户原话确定 `title`，并让页面标题、Skill 回复文本使用同一个最终标题。按以下顺序执行：

1. 从“查询、查看、获取、给我、给我看、看、拿到、发我”等查询/交付动作词之后提取主题短语；例如“查询 AI 初筛通过表”提取“AI 初筛通过表”。
2. 保留主题中的业务含义以及“表”“列表”“简历”等业务后缀，不要把它们裁掉。
3. 去掉句末交付动作词或礼貌用语，包括“给我链接”“发链接”“看一下”“谢谢”等；如果动作词前没有明确业务主题，不要把“这些简历”之类的指代当作标题。
4. 清洗标题：去除首尾空格、包裹引号和控制字符，将连续空白合并为一个空格，并按 Unicode 字符截取到最多 60 个字符。
5. 清洗后为空、只有动作词/交付用语，或无法识别明确业务主题时，使用默认标题“业务筛选”。

示例：

```text
查询 AI 初筛通过表 → title: AI 初筛通过表
给我看魏秋柠待筛选简历 → title: 魏秋柠待筛选简历
把这些简历发给面试官 → title: 业务筛选
```

业务筛选链接使用现有推送接口：

```http
POST /api/resumes/business-screening/push
Content-Type: application/json
x-api-key: <RESUME_UPLOAD_API_KEY>
```

请求体中的 `title` 使用上面的提取结果；`subtitle` 沿用业务筛选页面的决策提示：
...
```json
{
  ...
  "expires_in_days": 30,
  "title": "AI 初筛通过表",
  "subtitle": "请查看候选人信息并完成入库 / 不入库决策。"
}
```

回复用户时使用最终标题和链接，例如：`AI 初筛通过表：<链接>`；没有明确主题时使用：`业务筛选：<链接>`。文档中只保留 `<RESUME_UPLOAD_API_KEY>` 占位符，不写入真实密钥。
