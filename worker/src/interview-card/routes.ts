...
async function loadCardContext(env: any, rawToken: string, deps: InterviewCardRouteDeps) {
  ...
  // Keep links generated by the previous reminder implementation readable. Those
  // rows live in interview_card_links and intentionally do not have interview_id.
  // The fallback is read-only and only selects an exact candidate/position match.
  if (!card) {
    const legacy = await db.prepare(
      'SELECT id, token_hash, resume_id, candidate_name, position_applied, status, expires_at, created_at FROM interview_card_links WHERE token_hash = ? LIMIT 1',
    ).bind(tokenHash).first() as Row | null;
    if (legacy) {
      const legacyInterview = await db.prepare(
        'SELECT id FROM interviews WHERE candidate_name = ? AND position_applied = ? ORDER BY created_at DESC LIMIT 1',
      ).bind(legacy.candidate_name || '', legacy.position_applied || '').first() as Row | null;
      card = {
        ...legacy,
        interview_id: legacyInterview?.id || null,
      };
    }
  }
